...
if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['id_reporte_recoleccion'])) {
    ...
    if ($id_reporte > 0 && !empty($accion_admin)) {
        ...
        if (!$stmt_get_info) {
             $_SESSION['mensaje_accion_recoleccion'] = ['tipo' => 'error', 'texto' => "Error al preparar consulta de reporte: " . $conn->error];
        ...
        if (!empty($nuevo_estado)) {
            ...
            try {
                ...
                // CORRECCIÓN FINAL: Aseguramos la columna 'fecha_hora' si existe, o la quitamos si no.
                // Asumo que tu tabla 'auditoria' tiene una columna 'fecha_hora' de tipo TIMESTAMP o DATETIME.
                // Si la columna 'fecha_hora' no existe en tu DB, por favor, elimínala de la sentencia INSERT y del bind_param.
                // O, MEJOR AÚN, AGREGA LA COLUMNA A TU TABLA DE AUDITORÍA:
                // ALTER TABLE auditoria ADD COLUMN fecha_hora TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

                $id_admin_realiza_accion = $_SESSION['user_id']; // El ID del admin que realiza la acción
                $tabla_modificada_audit = 'reportes_recoleccion_usuario';
                $id_registro_modificado_audit = $id_reporte;
                $detalles_accion_audit = "Reporte Recolección ID: {$id_reporte}. Usuario: {$id_usuario_reporte}. Puntos: {$puntos_finales}. Observaciones: {$observaciones_admin}. Validado por Admin ID: {$id_admin_realiza_accion}.";
                $ip_origen_auditoria = $_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN_IP';
                $fecha_hora_actual_audit = date('Y-m-d H:i:s'); // Obtener fecha y hora actual

                // He modificado la consulta INSERT para incluir id_usuario_admin.
                // Si tu tabla de auditoría NO TIENE 'id_usuario_admin', cámbiala a:
                // "INSERT INTO auditoria (id_usuario_afectado, accion_realizada, tabla_modificada, id_registro_modificado, detalles_accion, ip_origen, fecha_hora) VALUES (?, ?, ?, ?, ?, ?, ?)"
                // y luego id_usuario_afectado será el ID del admin que realiza la acción.
                // Pero lo más limpio es tener id_usuario_admin e id_usuario_afectado separados.
                
                // Opción 1: Si tienes `id_usuario_admin` en tu tabla `auditoria` (RECOMENDADO)
                $stmt_audit = $conn->prepare("INSERT INTO auditoria (id_usuario_admin, id_usuario_afectado, accion_realizada, tabla_modificada, id_registro_modificado, detalles_accion, ip_origen, fecha_hora) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                if (!$stmt_audit) {
                    throw new Exception("Error al preparar la consulta de auditoría: " . $conn->error);
                }
                $stmt_audit->bind_param(
                    "sssssssi", // 7 strings, 1 integer (para id_usuario_afectado)
                    $id_admin_realiza_accion,    // ID del admin que realiza la acción
                    $id_usuario_reporte,         // ID del usuario afectado por la recolección
                    $log_accion,
                    $tabla_modificada_audit,
                    $id_registro_modificado_audit,
                    $detalles_accion_audit,
                    $ip_origen_auditoria,
                    $fecha_hora_actual_audit     // Fecha y hora
                );
                
                // Opción 2: Si NO tienes `id_usuario_admin` y solo usas `id_usuario_afectado` para registrar al admin
                // (Descomenta esto y comenta la Opción 1 si tu DB no tiene id_usuario_admin)
                /*
                $stmt_audit = $conn->prepare("INSERT INTO auditoria (id_usuario_afectado, accion_realizada, tabla_modificada, id_registro_modificado, detalles_accion, ip_origen, fecha_hora) VALUES (?, ?, ?, ?, ?, ?, ?)");
                if (!$stmt_audit) {
                    throw new Exception("Error al preparar la consulta de auditoría: " . $conn->error);
                }
                $stmt_audit->bind_param(
                    "sssssss", // 7 strings
                    $id_admin_realiza_accion,    // Aquí va el ID del admin
                    $log_accion,
                    $tabla_modificada_audit,
                    $id_registro_modificado_audit,
                    $detalles_accion_audit,
